...
审核他人已完成的 5Why 分析：**一次性**填写问题背景 + 逐层「为什么 / 分析」（可扩充至最多 20 层，后几层可选，无需凑满），AI 逐层审核整条逻辑链是否存在漏洞（答非所问、跳层、猜测当事实、归咎个人、过早停止等），输出结论、漏洞清单、逐层点评与修改建议。
...
- 在表单中填写：问题背景（可选，建议填写）、第 1 层「为什么 + 分析」（必填）、点击「＋ 添加下一层」可继续扩充至最多 20 层（留空即忽略，每层右侧 × 可删除）、问题总结（必填）
...
- 表单审核：问题背景 + 1~20 层「为什么 / 分析」一次性提交（第 2 层起可选，填一半会提示补全），Enter 提交，Shift+Enter 换行
